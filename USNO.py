...
import requests
import json
R = requests.post('http://nova.astrometry.net/api/login', data={'request-json': json.dumps({"apikey": "kicxdposandvygae"})})
print(R.text)
import os
import requests
import json
...
def check_astrometry_status(job_id, session_key, output_dir):
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            if status == 'success':
                # Retrieve annotated image URL
                if 'url' in response_json:
                    annotated_image_url = response_json['url']
                    print("Annotated image URL:", annotated_image_url)
                    
                    # Download annotated image
                    download_image(annotated_image_url, output_dir)
                    
                    return True
                else:
                    print("Annotated image URL not found.")
                    return False
            elif status == 'failure':
...
def download_image(url, output_dir):
    ...
    filename = os.path.join(output_dir, 'annotated_image.jpg')
    try:
        with requests.get(url, stream=True) as r:
            with open(filename, 'wb') as f:
                shutil.copyfileobj(r.raw, f)
        print(f"Downloaded annotated image to: {filename}")
    except Exception as e:
        print("Failed to download annotated image:", str(e))

if __name__ == "__main__":
    # URL of the annotated image from Astrometry.net
    annotated_image_url = 'https://nova.astrometry.net/user_images/10197139#annotated'
    
    # Output directory for downloaded annotated image
    output_directory = r"E:\finalGPbegad\annotated_images"
    os.makedirs(output_directory, exist_ok=True)
    
    # Download annotated image
    download_image(annotated_image_url, output_directory)
# import numpy as np
# from scipy.ndimage import gaussian_filter, label
# from astropy.io import fits
# from astroquery.vizier import Vizier
